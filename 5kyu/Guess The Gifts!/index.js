...
Description: You will be given a wishlist (array), containing all possible items. Each item is in the format: {name: "toy car", size: "medium", clatters: "a bit", weight: "medium"} (Ruby version has an analog hash structure, see example below)
...
function guessGifts(wishlist, presents) {
  ...
  for (let i = 0; i < wishlist.length; i++) {
    for (let j = 0; i < presents.length; j++) {
      let present = wishlist[i];

      if (
        present.size === presents[j].size &&
        present.clatters === presents[j].clatters &&
        present.weight === presents[j].weight
      ) {
        arr.push(present.name);
        break;
      }
    }
  }
